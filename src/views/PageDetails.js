...
class PageDetails extends Component {
    ...
    render() {
        const { image, title, author, catalog } = this.props.item
        return (
	        <div className="column-flex" style={{ height: '100%', padding: 10 }}>
                <div className="row-flex detail-header">
                    <img className="detail-header-img" src={image} />
                    <div className="column-flex">
                        <div><span>名称: </span>{title}</div>
                        <div><span>作者: </span>{author.join(' ')}</div>
                        <div><span>出版社: </span></div>
                        <div><span>日期: </span></div>
                        <div><span>评分: </span></div>
                        <div><span>价钱: </span></div>
                    </div>
                </div>
                <div>
                    <div>序言</div>
